...
// ─── Imported products (generated by scripts/import-products.ts) ────────────
...
export function getImportedProductsByCategory(category: string): ImportedProduct[] {
  if (category === 'all') return importedProducts
  return importedProducts.filter((p) => p.category === category)
}

export function getImportedProductsBySubcategory(
  category: string,
  subcategory: string | null,
): ImportedProduct[] {
  if (category === 'all') return importedProducts
  const byCat = importedProducts.filter((p) => p.category === category)
...
export function getSubcategoryCounts(category: string): Record<string, number> {
  const byCat = category === 'all' ? importedProducts : importedProducts.filter((p) => p.category === category)
...
export function getCategoryCounts(): Record<string, number> {
  return importedProducts.reduce<Record<string, number>>((acc, p) => {
    acc[p.category] = (acc[p.category] ?? 0) + 1
    return acc
  }, {})
}

export function getImportedRelated(currentSlug: string, category: string, limit = 4): ImportedProduct[] {
  const same = importedProducts.filter((p) => p.slug !== currentSlug && p.category === category)
  return same.length >= limit ? same.slice(0, limit) : [
    ...same,
    ...importedProducts.filter((p) => p.slug !== currentSlug && p.category !== category),
  ].slice(0, limit)
}
