    ...
    <div className="relative border-2 border-dashed border-cream-300 rounded-3xl p-8 lg:p-10 bg-white">
      ...
      <form onSubmit={handleSubmit} noValidate>
        ...
        <div className="mt-8 flex flex-col items-center gap-3">
          ...
          {/* Trust reassurance — reduces submit anxiety */}
          <p className="text-xs text-ink-muted text-center max-w-sm">{t('reassurance')}</p>
        </div>

        {/* Error state — recover the lead with direct contact fallbacks */}
        {status === 'error' && (
          <div className="mt-5 rounded-xl bg-red-50 border border-red-100 p-4 text-center">
            <p className="text-sm text-red-600 mb-2">{t('errorText')}</p>
            <p className="text-xs text-ink-muted mb-3">{t('errorHelp')}</p>
            <div className="flex flex-wrap items-center justify-center gap-2.5">
              <a
                href="mailto:sales@potatoapparel.com"
                className="inline-flex items-center gap-1.5 bg-white border border-cream-300 hover:border-violet-300 text-ink text-xs font-semibold px-4 py-2 rounded-full transition-colors"
              >
